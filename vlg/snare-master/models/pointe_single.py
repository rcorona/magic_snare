...
class PointeClassifier(LightningModule):
    ...
    def forward(self, batch):
        ...
        # obj1_enc = latent1.max(-2)[0]
        # obj2_enc = latent2.max(-2)[0]
        obj1_enc = latent1
        obj2_enc = latent2
        ...
        # feats1 = torch.cat([obj1_enc.max(1)[0], vl1_feats], dim=-1)
        # feats2 = torch.cat([obj2_enc.max(1)[0], vl2_feats], dim=-1)
        # feats1 = torch.cat([obj1_enc, vl1_feats], dim=-1)
        # feats2 = torch.cat([obj2_enc, vl2_feats], dim=-1)
        # Score each object. 
