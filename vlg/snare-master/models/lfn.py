...
class LFNClassifier(LightningModule):
    ...
    def transformer_pass(self, feats, padding_mask, get_weights=False): 
        ...
        feats = feats + positional_encoding
    ...
    def forward(self, batch):
        ...
        Data unpacking. 
        """
        obj1_n_feats, obj2_n_feats = batch['obj_feats']
        ...
        feats1 = torch.cat([obj1_enc, cls_token, lang_enc], dim=1)
        feats2 = torch.cat([obj2_enc, cls_token, lang_enc], dim=1)

        feats1 = self.transformer_pass(feats1, padding_mask)
        feats2 = self.transformer_pass(feats2, padding_mask)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(len(annotations)):
            ...
            item = {
                ...
                'obj_0':obj_key_0[i], 
                'obj_1':obj_key_1[i], 
                'probs':probs[i].cpu().detach().numpy(), 
                'labels':labels[i].cpu().detach().numpy(), 
                'is_visual':visual[i].cpu().detach().numpy()
            }
